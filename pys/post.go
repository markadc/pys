...
func POST(URL string, headers, params S, data A) (*Response, error) {
	// 请求体
	jsonBytes, err := json.Marshal(data)
	if err != nil {
		return nil, err
...
func POST2(URL string, headers, params S, bodys []byte) (*Response, error) {
	// 构造请求
	req, err := http.NewRequest("POST", MakeURL(URL, params), bytes.NewBuffer(bodys))
	if err != nil {
		return nil, err
	}
